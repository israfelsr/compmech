...
class AttributeProbes:
    ...
    def train_single_probe(self, features: np.ndarray, labels: np.ndarray, 
                          attribute_idx: int, cv_folds: int = 5, 
                          n_repeats: int = 2) -> Optional[Dict]:
        """
        ...
        if np.sum(y) < cv_folds or np.sum(1-y) < cv_folds:
            logging.warning(f"Skipping attribute {attribute_idx}: insufficient examples")
            return None
        ...
        for repeat in range(n_repeats):
            ...
            for train_idx, val_idx in skf.split(features, y):
                ...
                # Calculate metrics
                all_scores['f1'].append(f1_score(y_val, y_pred, average='binary', zero_division=0))
                all_scores['accuracy'].append(accuracy_score(y_val, y_pred))
                all_scores['precision'].append(precision_score(y_val, y_pred, average='binary', zero_division=0))
                all_scores['recall'].append(recall_score(y_val, y_pred, average='binary', zero_division=0))
    ...
    def train_all_probes(self, features: np.ndarray, labels: np.ndarray, 
                        cv_folds: int = 5, n_repeats: int = 2) -> Dict:
        """
        ...
        for attr_idx in tqdm(range(num_attributes), desc="Training attribute probes"):
            results = self.train_single_probe(features, labels, attr_idx, cv_folds, n_repeats)
            if results is not None:
                all_results.append(results)
                logging.info(f"Attribute {attr_idx}: F1 = {results['mean_f1']:.4f} ± {results['std_f1']:.4f}")
    ...
    def evaluate_specific_attributes(self, features: np.ndarray, labels: np.ndarray,
                                   attribute_indices: List[int], cv_folds: int = 5, 
        ...
        """
        logging.info(f"Training probes for {len(attribute_indices)} specific attributes...")
        
        all_results = []
        
        for attr_idx in tqdm(attribute_indices, desc="Training specific attribute probes"):
            results = self.train_single_probe(features, labels, attr_idx, cv_folds, n_repeats)
            if results is not None:
                all_results.append(results)
                logging.info(f"Attribute {attr_idx}: F1 = {results['mean_f1']:.4f} ± {results['std_f1']:.4f}")
    ...
    def _create_probe(self):
        ...
        if self.probe_type == 'logistic':
            ...
            return MLPClassifier(hidden_layer_sizes=(100,), max_iter=1000, random_state=self.random_seed)
        else:
...
class TorchAttributeProbes:
    ...
    def __init__(self, dataset=None, hidden_dims=[512, 256], dropout=0.1, 
                 learning_rate=0.001, batch_size=64, epochs=100, device='auto'):
        self.dataset = dataset
    ...
    def train_single_probe(self, features: torch.Tensor, labels: torch.Tensor,
                          attribute_idx: int, cv_folds: int = 5) -> Optional[Dict]:
        ...
        for train_idx, val_idx in skf.split(features.numpy(), y.numpy()):
            ...
            model = MLPProbe(features.shape[1], self.hidden_dims, 1, self.dropout).to(self.device)
